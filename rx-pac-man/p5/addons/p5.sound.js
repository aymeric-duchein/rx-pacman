...
shims = function () {
  ...
  */
  ...
  var isAACSupported = function () {
    return !!el.canPlayType && (el.canPlayType('audio/column-m4a;') || el.canPlayType('audio/aac;'));
  };
  var isAIFSupported = function () {
    return !!el.canPlayType && el.canPlayType('audio/column-aiff;');
  };
...
soundfile = function () {
   ...
   *  function draw() {
   *    background(0);
   *    ball.column = constrain(mouseX, 0, width);
   *    ellipse(ball.column, height/2, 20, 20)
   *  }
   *
   *  function mousePressed(){
   *    // map the ball's column location to a panning degree
   *    // between -1.0 (left) and 1.0 (right)
   *    var panning = map(ball.column, 0., width,-1.0, 1.0);
...
fft = function () {
   ...
   *      var column = map(i, 0, spectrum.length, 0, width);
   *      var h = -height + map(spectrum[i], 0, 255, height, 0);
   *      rect(column, height, width / spectrum.length, h )
   *    }
   *
   ...
   *      var column = map(i, 0, waveform.length, 0, width);
  ...
  /**
     ...
     *  //draw the spectrum
     *  for (var i = 0; i< spectrum.length; i++){
     *    var column = map(log(i), 0, log(spectrum.length), 0, width);
     *    var h = map(spectrum[i], 0, 255, 0, height);
     *    var rectangle_width = (log(i+1)-log(i))*(width/log(spectrum.length));
     *    rect(column, height, rectangle_width, -h )
     *  }
...
panner3d = function () {
   ...
   * @param  {Number} zVal
   * @param  {Number} time
   * @return {Array}      Updated column, y, z values as an array
   */
